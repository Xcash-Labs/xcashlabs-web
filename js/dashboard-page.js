...
const connectedAddress = ethers.getAddress(
...
/*
 * Verify the exact chain and native balance MetaMask is using
 * for this claim.
 */
const injectedChainId = await window.ethereum.request({
  method: 'eth_chainId'
});
...
const nativeBalanceEth = ethers.formatEther(
  nativeBalanceWei
);

console.group('[wXCK Claim Network Check]');
console.log('Request network:', request.network);
console.log('Expected chain name:', claimChain.chainName);
console.log('Expected chain ID hex:', claimChain.chainId);
console.log(
  'Expected chain ID decimal:',
  Number(BigInt(claimChain.chainId))
);
console.log('MetaMask chain ID hex:', injectedChainId);
console.log(
  'MetaMask chain ID decimal:',
  Number(BigInt(injectedChainId))
);
console.log(
  'Ethers provider chain ID:',
  ethersNetwork.chainId.toString()
);
console.log('Connected account:', connectedAddress);
console.log('Expected recipient:', expectedAddress);
console.log(
  `${claimChain.chainName} native balance:`,
  nativeBalanceEth
);
console.groupEnd();

if (
  injectedChainId.toLowerCase() !==
  claimChain.chainId.toLowerCase()
) {
  throw new Error(
    `MetaMask reports chain ${injectedChainId}, but this claim ` +
    `requires ${claimChain.chainName} (${claimChain.chainId}).`
  );
}

if (
  ethersNetwork.chainId !==
  BigInt(claimChain.chainId)
) {
  throw new Error(
    `The ethers provider reports chain ` +
    `${ethersNetwork.chainId.toString()}, but this claim requires ` +
    `${claimChain.chainName}.`
  );
}

if (connectedAddress !== expectedAddress) {
  throw new Error(
    `Wrong MetaMask account. Please switch MetaMask to ` +
    `${claim.recipient} and try again.`
  );
}

if (nativeBalanceWei === 0n) {
  throw new Error(
    `The connected account has no ETH on ${claimChain.chainName}.\n\n` +
    `Base requires ETH on Base to pay the claim transaction fee. ` +
    `ETH on Ethereum or assets on Polygon cannot pay this fee.`
  );
}
